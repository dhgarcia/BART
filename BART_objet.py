# "C:\Program Files (x86)\Python27\python" C:\Users\erohart\Desktop\BART\BART_objet.py
...
# path where results files will stocks
...
class BART_TestApp(App):
    ...
    def change_value(self,btn):  # fonction called by the 'collect reward' button
        if (self.reward.opacity != 0):
            ...
            if (self.nb_bal>=29): # if  30 balloons have been used
                ...
                self.box4.add_widget(Button(text='close the BART test',size_font='20sp',size_hint=(None,None),size=(200,100),pos_hint={'center_x':0.5,'center_y':0.25},on_press=self.close))
                ...
                self.maxi = randint(1,127) # new maximum value
    ...
    def blow_up(self,btn) :   # fonction called by the 'blow up' button
        ...
        if (self.blow.text == "new balloon"): # if the balloon just come to explose
            ...
            if ((self.nb_bal==29 and self.nb_pump>=self.maxi) or (self.nb_bal>=30)): # if 30 balloons have been used
                ...
                self.files.write("%d , 0 , 0 , %d , %d , 1,  %.2f \n" % (self.nb_bal+1,self.nb_pump,self.maxi,self.p))
                self.files.write("score : %.2f \n" % finalvalue)
                deb2 = time.time()  # take the time  
                self.deb = int((deb2 - self.deb)*1000) 
                self.tabms += [int(self.deb)]  
                self.deb = deb2
                l = len(self.tabms)  
                self.tabal += [l]   
                ...
                self.nb_pump=0
                self.nb_bal+=1
                x = 0.18 # resize the balloon image
                y = 0.18
    ...
    def quits(self,btn):  # fonction called by the 'quit' button, asks a confirmation before quit the game
        ...
        self.box.add_widget(Button(text='yes',size_hint=(None,None),size=(120,100),pos_hint={'center_x':0.25,'center_y':0.3},on_press=self.yes)) # yes button
        self.box.add_widget(Button(text='no',size_hint=(None,None),size=(120,100),pos_hint={'center_x':0.75,'center_y':0.3},on_press=self.no)) # no button
