...
import naoqi, sys, argparse, motion, almath                  # imports for the nao part
from naoqi import *
...
class BART_TestApp(App):
    ...
    def build(self):
        ...
        self.deb = time.time() # note the time pass between two clicks
        ...
        self.t = 0 # time between two nao's sentence
        ...
        for i in range(0,6):
            self.z[i] = randint(1,29) # choose a balloon for the robot's intervention
            j = 0
            while (j<i): # for all value completes on the table
                if (self.z[j] == self.z[i]) : # if the balloon has already a robot's intervention
                    self.z[i] = randint(1,30) # choose a new balloon
                    j = 0 # comback at the first value of the table
                else :
                    j+=1 # continue to checkballoon's values on the table 
        self.files = open(self.ID,"a") # open the text file     
        self.files.write("balloon number,color of the balloon[1:red(1,40)/2:green(35,84)/3:blue(80,127)],robot intervention [0/1],number of verbal interventions,number of pumps, explosion point, explosion [0:no/1:yes], payment \n")
    ...
    def speak(self,sentence): # nao speech with no blockage of the rest of the application
        self.animated = ALProxy("ALAnimatedSpeech") # intializations
        self.config={"bodyLanguageMode":"contextual"}
        threading.Thread(target=self.animated.say,args=(sentence,self.config)).start() # nao's speech
    ...
    def change_value(self,btn):  # fonction called by the 'collect reward' button
        if (self.reward.opacity != 0):
            ...
            if (self.nb_bal>=29): # if  30 balloons have been used
                ...
                self.files.write("%d , %d , %d , %d , %d , %d , 0 , %.2f \n" % (self.nb_bal+1,self.color,self.b,self.s,self.nb_pump,self.maxi,self.p))     
                ...
                elif (self.color == 3):  # blue balloon
    ...
    def talk(self,btn):  # Nao movements and talking after an explosion
        JointNames = ["RShoulderPitch", "RShoulderRoll","LShoulderPitch", "LShoulderRoll"] # imitialisation for right and left arms
    ...
    def blow_up(self,btn) :   # fonction called by the 'blow up' button
        ...
        if (self.blow.text == "new balloon"): # if the balloon just come to explose
            ...
            if ((self.nb_bal>=29 and self.nb_pump>=self.maxi) or (self.nb_bal >=30)):  # if 30 balloons have been used
                finalvalue = ((self.bar.value)*100)/30 # cange value in percentage
                ...
                if (self.nb_pump == 0):# if it's the first pump for this balloon
                ...
                if (self.b == 1) : # ask if nao is in careful mode
                    ...
                    if ((self.nb_pump >= 5) and (self.delivery2-self.delivery >=self.t)): # if we are with a low balloon
                        ...
                        if (alea == 1):
                            ...
                            self.t = 2 # time before an other sentence
                        elif(alea == 2):
