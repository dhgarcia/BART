...
class BART_TestApp(App):
    ...
    def build(self):
        ...
        self.last_money = Label(text='0.00', font_size='20dp',pos_hint={'center_x':0.94,'center_y':0.35})
        game.add_widget(self.last_money) # last balloon

        self.points = Label(text='total points earned:', font_size='20dp',pos_hint={'center_x':0.8,'center_y':0.4})
        game.add_widget(self.points) # points

        self.last_points = Label(text='points last balloon:', font_size='20dp',pos_hint={'center_x':0.8,'center_y':0.35})
        game.add_widget(self.last_points) # last points
    ...
    def change_value(self,btn):  # fonction called by the 'collect reward' button
        if (self.reward.opacity != 0):
            ...
            r = 0
            self.p = self.p+(1*self.nb_pump) # money earn by participant
            if (len(str(self.p)) <4):
                self.money.text = str(self.p)+'0'
                self.last_money.text = str(1.0*self.nb_pump)+'0'
            else:
                self.money.text = str(self.p)
                self.last_money.text = str(1.0*self.nb_pump)
...
#                    elif (alea == 7):
#                        self.speak("you've earned "+str(self.p)+"point")
#                    elif (alea == 8):
#                        self.speak("fabulous")
#                    elif (alea == 9):
...
#                        self.speak("I will do an heart attack one day ")
    ...
    def talk(self,v): # Nao movements and talking after an explosion
        ...
        #elif (alea == 8):
        #    self.speak("you just come to lose" + str((self.nb_pump-1)*1)+"points")
        #elif (alea == 9):
...
#                            self.t = 2.5
    ...
    def open_rules(self,btn):  # fonction called by the 'rules' button
        popup = Popup(title='rules',title_align ='center',title_size = '30sp', content=Label(text='''
            ...
            Each pump is worth 1 point: the more you pump, the more you earn!
