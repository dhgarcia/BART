...
import naoqi, sys, argparse, motion, almath                 # imports from the nao part
from naoqi import *
...
class BART_TestApp(App):
    ...
    def build(self):
        ...
        self.nb_bal = 0 # nomber of used balloon
        self.t = 3 # time between two nao's sentences
        self.color = 3 # color of the balloon (1 : red,2 : green,3 : blue)
        self.maxi = randint(80,127) # random fonction for the maximum pumping of the first balloon
    ...
    def speak(self,sentence):
        tts = ALProxy("ALTextToSpeech",IP,PORT) # nao speech with no blockage of the rest of the application
        animated = ALProxy("ALAnimatedSpeech",IP,PORT) # intializations
    ...
    def talk(self,v):  # Nao movements and talking after an explosion
        JointNames = ["RShoulderPitch", "RShoulderRoll","LShoulderPitch", "LShoulderRoll"] # initialisation for right and left arms
        ...
        if (self.nb_bal > 29): # it's the end of the game
            self.speak("It doesn't matter \\pau=750\\ this is the end of the game \\pau=500\\ Thank you !")
        elif (alea == 1):
            self.speak("oups !")
        elif (alea == 2):
    ...
    def blow_up(self,btn) :   # fonction called by the 'blow up' button
        ...
        if (self.blow.text == "new balloon"):  # if the balloon just come to explose
            ...
            if (self.color == 1):
                self.im_bal.source = "images/deflated_red.jpg"
                self.maxi = randint(1,40)
            elif (self.color == 2):
                self.im_bal.source = "images/deflated_green.jpg"
                self.maxi = randint(35,85)
            else :
                self.im_bal.source = "images/deflated_balloon.jpg"
                self.maxi = randint(80,127)
            ...
            if ((self.nb_bal==29 and self.nb_pump>=self.maxi) or (self.nb_bal >=30)): # if 30 balloons have been used
                ...
                self.nb_pump+=1
                self.sound_pump.play() # a pump sound
    ...
    def quits(self,btn): # fonction called by the 'quit' button, asks a confirmation before quit the game
        ...
        self.tts.say("Are you sure you want to quit me ?")
    ...
    def yes(self,btn):   # if the participant wants quit the game before the end
        ...
        popup = Popup(title='END',title_align ='center',title_size = '30sp',content=self.box2,size_hint = (0.7,0.7),auto_dismiss=False) 
